...
def report_user(req, user: User, task_id, user_id):
    if req.method == "POST":
        ...
        if tagger is None or (
                task.current_tag_user_list.filter(tag_user=tagger).first() is None and task.publisher != tagger
        ):
            return request_failed(35, "user is not demand of this task", 404) if task.publisher != tagger else \
        ...
        if report_info is None:
            ReportInfo.objects.create(task=task, user=tagger, reason=reason)
